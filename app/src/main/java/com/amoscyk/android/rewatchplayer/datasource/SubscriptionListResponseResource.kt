...
class SubscriptionListResponseResource private constructor(
    private val request: YouTube.Subscriptions.List
) {
    private val _resource = MutableLiveData<Resource<List<RPSubscription>>>()
        .apply { Resource.loading(null) }
    val resource: LiveData<Resource<List<RPSubscription>>> = _resource
    ...
    private suspend fun startRequest() {
        withContext(Dispatchers.IO) {
            ...
            Log.d("LOG", "start subscription request")
            try {
                val response = request.execute()
                Log.d("LOG", "finish subscription request")
                val result = response.toRPSubscriptionListResponse("")
    ...
    suspend fun loadMoreResource() {
        ...
        withContext(Dispatchers.IO) {
            ...
            Log.d("LOG", "start load more subscription request")
            try {
                val response = request.setPageToken(_pageToken).execute()
                Log.d("LOG", "finish load more subscription request")
                val result = response.toRPSubscriptionListResponse(_pageToken!!)
                _pageToken = result.nextPageToken
                if (_pageToken == null) {
                    _endOfListReached = true
                }
                val newList = ArrayList<RPSubscription>().apply {
    ...
    companion object {
        suspend fun build(request: YouTube.Subscriptions.List): SubscriptionListResponseResource {
            val resource = SubscriptionListResponseResource(request)
